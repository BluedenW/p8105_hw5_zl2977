...
the true \(\mu=0\), and then gradually approaches 1 as the the true
\(\mu\) increases towards 6.
...
As the above plot shows, the sample averages of all estimated
\(\hat{\mu}\) are approximately equal to the true \(\mu\). While the
sample averages of \(\hat{\mu}\) across tests for which the null is
rejected are not approximated equal to the true \(\mu\).

Because `rej_mu_hat` contains \(\hat{\mu}\) only in samples for which
null was rejected, it appears more far away from the null comparing with
the values of `all_mu_hat` since `all_mu_hat` also contains samples
which were failed to reject. Therefore the values of `all_mu_hat` are
...
significantly far way values appears much greater than the average of
all estimates at beginning (except true \(\mu=0\) because in this case
the rejected values appeared by equal chance in both sides of the
null=0, the average of these rejected value thus close to the true
\(\mu\)). However, when the true \(\mu\) increases, the majority of the
estimates are far away from the null (which is 0), in other words, the
majority of the estimate values were being rejected, thus the difference
of average between all estimates and estimates being rejected became
smaller, resulting two lines got closer in the end.
