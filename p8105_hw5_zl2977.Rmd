...
#   %>%
#   ggplot(aes(x=true_mu, y =ave_mu_hat))+
#   geom_path()
# est_all_mu_hat
```
...
```
As the above plot shows, the sample averages of all estimated $\hat{\mu}$ are approximately equal to the true $mu$. While the sample averages of $\hat{\mu}$ across tests for which the null is rejected are not approximated equal to the true $\mu$.

Because `rej_mu_hat` contains $\hat{\mu}$ only in samples for which null was rejected, it appears more far away from the null comparing with the values of `all_mu_hat` since `all_mu_hat` also contains samples which were failed to reject. Therefore the values of `all_mu_hat` are represented closer to the true $\mu$ values.

When the true $\mu$ are small, only the estimates that are significant far way from the null can be rejected, thus the average of these significantly far way values appears much greater than the average of all estimates at beginning. However, when the true $\mu$ increases, the majority of the estimates are far away from the null (which is 0), in other words, the majority of the estimate values were being rejected, thus the difference of average between all estimates and estimates being rejected became smaller, result two lines got closer in the end.